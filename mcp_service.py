...
class MCPService:
    ...
    def __init__(self):
        ...
        logger.info(f"Initialized MCPService with servers: {self.mcp_servers}")
    
    async def _get_tools_from_server(self, server_url: str) -> List[dict]:
        """Get tools from a server and map them"""
        try:
            async with streamablehttp_client(f"{server_url}/mcp", auth=None) as (read, write, _):
                async with ClientSession(read, write) as session:
                    ...
                    for tool in tools.tools:
                        self.server_tool_map[tool.name] = server_url
                        result.append({
                            "type": "function",
                            "function": {
                                "name": tool.name,
                    ...
                    logger.info(f"{server_url}: {len(result)} tools")
                    return result
        except Exception as e:
            logger.warning(f"{server_url}: {e}")
            return []
    
    async def _execute_tool(self, tool_name: str, args: dict) -> str:
        """Execute tool on its server"""
        server_url = self.server_tool_map.get(tool_name)
        if not server_url:
    ...
    async def process_with_mcp(self, user_prompt: str) -> Optional[List[Message]]:
        ...
        try:
            ...
            for server in self.mcp_servers:
                all_tools.extend(await self._get_tools_from_server(server))
