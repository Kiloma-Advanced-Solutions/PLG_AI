...
python3 python_mcp_server.py > /tmp/python_mcp.log 2>&1 &
PY_PID=$!

# Start Chat API in background
echo "▶️  Starting Chat API (port 8001)..."
python3 -m uvicorn chat_api:app --reload --port 8001 > /tmp/chat_api.log 2>&1 &
API_PID=$!
...
check_server "http://localhost:8002/health" "Python MCP Server" && echo "✅ Python MCP Server (port 8002)"
