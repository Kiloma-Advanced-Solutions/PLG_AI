...
קריטריונים להערכה:
**1. שלמות המשימות** (0-40 נקודות): האם המערכת זיהתה נכונה את המשימות הקיימות ולא הוסיפה משימות שאינן קיימות? הציון צריך להביע בצורה מדויקת ככל הניתן את אחוז הדיוק של המערכת בזיהוי המשימות הקיימות.
**2. איכות השדות** (0-60 נקודות): האם כל השדות (שולח המשימה, תאריכים, האדם שצריך לבצע את המשימה, וכו') מולאו בצורה נכונה? לא נדרש להיות דיוק מושלם, אלא נכונות בפרטי השדות באופן כללי (במידה ואין משימות קיימות, ציון מלא יינתן אם המערכת לא מילאה את השדות כלל)
חשוב שתהיה ביקורתי והוגן בהערכתך. הורד ניקוד אם המערכת לא הצליחה לזהות את כל המשימות, אם היא חילצה משימות שלא קיימות, או אם היא מילאה את השדות בצורה לא מדויקת.
...
הערות: [הסבר מדוע הענקת את הציון שבחרת. צריך להכיל הסבר על הסיבות להורדת הניקוד, אם יש כאלו]
...
{json.dumps(extracted_tasks, ensure_ascii=False, indent=2, default=str)}
    ...
    async def judge_extraction(self, email_content: str, extracted_tasks: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "score": score,
                "judgment": judgment,
                "tasks_extracted_by_judge": tasks_extreacted_by_judge,
    ...
    async def test_single_email(self, test_case: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"---Tasks Extracted By the Model---")
